...
const Sidebar = () => {
  ...
  const [showSisebar, setShowSidebar] = useState(false);
  ...
  if (!showSisebar) {
    return (
      <div>
        <Hamburger
          className="fixed md:hidden m-2 h-6 w-6 text-gray-900 cursor-pointer z-30"
          onClick={() => setShowSidebar(!showSisebar)}
        />
      </div>
    );
  }
  ...
  return (
    <>
      {/* Overlay */}
      <div
        className="md:hidden bg-slate-900/60 w-full h-screen fixed top-0 left-0 z-20"
        onClick={() => setShowSidebar(false)}
      />
      {/* Sidebar */}
      <div className="fixed left-0 top-0 p-4 z-30 bg-gray-900 text-slate-100 w-64 min-h-screen">
        <div className="mb-8">
          <div className="flex items-center mb-6">
            <p className="text-lg font-semibold">Inventory App</p>
            <BarChart3 className="ml-2 h-6 w-6" />
            <XCircle
              className="md:hidden ml-2 h-6 w-6 absolute top-2 right-2 cursor-pointer"
              onClick={() => setShowSidebar(!showSisebar)}
            />
          </div>
